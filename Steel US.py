...
feature_cols = ['S_MA5', 'S_MA20', 'USD/TWD', 'Oil Price', 'M2',
                'Discount Rate', 'VIX index', 'CPI index', 'Log Return', 'Target', 'US_EPU']
...
# === 5. Add standardized US_EPU ===
US_EPU_scaled = StandardScaler().fit_transform(df['US_EPU'].values.reshape(-1, 1))
US_EPU_df = pd.DataFrame(US_EPU_scaled, columns=['US_EPU'])

# === 6. Combine PCA features + US_EPU ===
X_all = pd.concat([pca_top, US_EPU_df], axis=1).astype('float32')
...
print("\n Average Model Performance (PCA + US_EPU):")
