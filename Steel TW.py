...
feature_cols = ['S_MA5', 'S_MA20', 'USD/TWD', 'Oil Price', 'M2',
                'Discount Rate', 'VIX index', 'CPI index', 'Log Return', 'Target', 'TW_EPU']
...
# === 5. Add standardized TW_EPU ===
TW_EPU_scaled = StandardScaler().fit_transform(df['TW_EPU'].values.reshape(-1, 1))
TW_EPU_df = pd.DataFrame(TW_EPU_scaled, columns=['TW_EPU'])

# === 6. Combine PCA features + TW_EPU ===
X_all = pd.concat([pca_top, TW_EPU_df], axis=1).astype('float32')
...
print("\n Average Model Performance (PCA + TW_EPU):")
