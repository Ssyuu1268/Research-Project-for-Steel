...
feature_cols = ['S_MA5', 'S_MA20', 'USD/TWD', 'Oil Price', 'M2',
                'Discount Rate', 'VIX index', 'CPI index', 'Log Return', 'Target', 'US-China Tension']
...
# === 5. Add standardized US-China Tension ===
Tension_scaled = StandardScaler().fit_transform(df['US-China Tension'].values.reshape(-1, 1))
Tension_df = pd.DataFrame(Tension_scaled, columns=['US-China Tension'])

# === 6. Combine PCA features + US-China Tension ===
X_all = pd.concat([pca_top, Tension_df], axis=1).astype('float32')
...
print("\n Average Model Performance (PCA + US-China Tension):")
